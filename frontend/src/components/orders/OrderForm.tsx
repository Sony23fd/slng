...
function getMaterialType(matName?: string, notes?: string): {
  ...
  if (n.includes('форзац') || note.includes('форзац')) {
    ...
    return { isAux: true, type: 'endpaper_plain', isNonPrinted: true };
  }
  if (n.includes('капитал') || note.includes('капитал')) {
    return { isAux: true, type: 'capital', isNonPrinted: true };
  }
  if (n.includes('хавчуурга') || note.includes('хавчуурга')) {
    return { isAux: true, type: 'ribbon', isNonPrinted: true };
  }
  if (n.includes('оосор') || note.includes('оосор')) {
    return { isAux: true, type: 'strap', isNonPrinted: true };
  }
  if (n.includes('бүрэлт') || note.includes('бүрэлт')) {
    return { isAux: true, type: 'coating', isNonPrinted: false };
  }
...
function getDefaultPrintSize(category?: string, productSize?: string, isCover?: boolean, bindingType?: string, coverRules?: any[]): string {
  ...
  const s = (productSize || 'A5').trim().toUpperCase();
  const bt = (bindingType || '').trim();

  if (cat === 'Түргэн хэвлэл') return 'A3';
  if (cat === 'Тор' || cat === 'Цаасан тор') return 'B2';

  if (isCover) {
    const coverLogic = getCoverLogic(s, bt, coverRules);
    if (coverLogic?.printSize) return coverLogic.printSize;
  }
...
export default function OrderForm({ initialData, isEdit, orderId, isQuoteMode }: { initialData?: any, isEdit?: boolean, orderId?: number, isQuoteMode?: boolean }) {
  ...
  const evaluateOperationFormula = (expression: string, overrideMaterials?: any[], overrideValues?: any) => {
    try {
      const cat = overrideValues?.category || getValues('category');
      const categoryConfig = productCategories.find((c: any) => c.name === cat) || {};
      ...
      const scope = {
        total_qty: Number(overrideValues?.total_qty || getValues('total_qty')) || 0,
        total_pages: Number(overrideValues?.total_pages || getValues('total_pages')) || 0,
        waste_qty: Number(categoryConfig.waste_qty) || 0,
        cover_colors: getColorsCount(overrideValues?.cover_color || getValues('cover_color') || ''),
        inner_colors: getColorsCount(overrideValues?.inner_color || getValues('inner_color') || ''),
        press_sheet,
        cover_sheets,
        inner_base_sheets,
        cover_base_sheets,
  ...
  const applyFullTemplate = (t: any) => {
    ...
    if (t.order_data) {
      ...
      let smartMaterials: any[] = [];
      if (od.materials && Array.isArray(od.materials)) {
        const a7 = (t.size || getValues('size') || 'A5').trim().toUpperCase();
        const category = (t.category || getValues('category') || '').trim();
        const bt = (t.binding_type || getValues('binding_type') || '').trim();
        const b4 = Number(t.total_pages || od.specifications?.total_pages || getValues('total_pages')) || 0;

        smartMaterials = od.materials.map((m: any) => {
          ...
          let rowDivideBy = Number(m.divide_by) || 1;
          let rowPressSheet = String(m.press_sheet || '');
          let rowSize = m.size;
          let baseQty = Number(m.base_qty) || tQty;
          let extraQty = Number(m.extra_qty) || 0;
          ...
          if (aux.type === 'ctp') {
            ...
            if (aux.type === 'cardboard') {
              ...
              extraQty = 0;
              totalQty = tQty;
              sheetQty = Math.ceil(tQty / specs.endpaperDiv);
              ...
              extraQty = 0;
              totalQty = tQty;
              sheetQty = Math.ceil(tQty / specs.headbandDiv);
            } else if (aux.type === 'ribbon') {
              const specs = getHardcoverAuxiliarySpecs(a7);
              rowPressSheet = '';
              rowDivideBy = 1;
              baseQty = tQty;
              extraQty = 0;
              totalQty = tQty;
              sheetQty = Math.ceil(tQty * specs.ribbonLength);
            } else if (aux.type === 'strap') {
              rowPressSheet = '';
              rowDivideBy = 1;
              baseQty = tQty;
              extraQty = 0;
            ...
            if (coverLogic) {
              ...
              if (coverLogic.printSize) rowPrintSize = coverLogic.printSize;
              baseQty = tQty;
              extraQty = calculateMakeready(baseQty);
              const divs = calculatePaperDivision(rowPrintSize || 'A2', a7) || 1;
              const setups = calculateSetups(Number(rowPressSheet) || 1, divs);
              totalQty = (baseQty * (Number(rowPressSheet) || 1)) + (extraQty * setups);
              sheetQty = Math.ceil(totalQty / rowDivideBy);
            } else if (isCover) {
              rowPressSheet = String(m.press_sheet || '1');
              if (!rowPrintSize) rowPrintSize = getDefaultPrintSize(category, a7, true, bt, coverRules);
              const matDivs = calculatePaperDivision(rowSize, rowPrintSize);
              if (matDivs > 0) rowDivideBy = matDivs;
              baseQty = tQty;
              extraQty = calculateMakeready(baseQty);
              const divs = calculatePaperDivision(rowPrintSize || 'A2', a7) || 1;
              const setups = calculateSetups(Number(rowPressSheet) || 1, divs);
              totalQty = (baseQty * (Number(rowPressSheet) || 1)) + (extraQty * setups);
              sheetQty = Math.ceil(totalQty / rowDivideBy);
            } else if (isInnerPageMaterial(m, category)) {
              // ЗӨВХӨН ДОТОР ХУУДСАН ДЭЭР ХЭВЛЭЛИЙН ХУУДАС БОДОГДОНО!
              if (!rowPrintSize) rowPrintSize = getDefaultPrintSize(category, a7, false, bt, coverRules);
              const matDivs = calculatePaperDivision(rowSize, rowPrintSize);
              if (matDivs > 0) rowDivideBy = matDivs;
              const m4 = calculateInnerPressSheet(b4, rowPrintSize, a7);
              rowPressSheet = String(m4);
              baseQty = tQty;
              extraQty = calculateMakeready(baseQty);
              const divs = calculatePaperDivision(rowPrintSize || 'A2', a7) || 1;
              const setups = calculateSetups(m4, divs);
              totalQty = (baseQty * m4) + (extraQty * setups);
              sheetQty = Math.ceil(totalQty / rowDivideBy);
            } else {
              // 1-хуудаст бүтээгдэхүүн (Брошур, Тор, Флаер гэх мэт)
              if (!rowPrintSize) rowPrintSize = getDefaultPrintSize(category, a7, false, bt, coverRules);
              const matDivs = calculatePaperDivision(rowSize, rowPrintSize);
              if (matDivs > 0) rowDivideBy = matDivs;
              rowPressSheet = '1';
              if (category === 'Брошур' || category === 'Флаер') {
                const pDiv = calculatePaperDivision(rowPrintSize, a7) || 1;
                baseQty = Math.ceil(tQty / pDiv);
              } else {
                baseQty = tQty;
              }
              extraQty = calculateMakeready(baseQty);
              const setups = 1;
              totalQty = (baseQty * 1) + (extraQty * setups);
              sheetQty = Math.ceil(totalQty / rowDivideBy);
            }
          }
    ...
    materials.forEach((m, index) => {
      ...
      if (aux.isAux) {
        ...
        if (aux.type === 'endpaper_plain') {
          ...
          setValue(`materials.${index}.press_sheet`, '1');
          setValue(`materials.${index}.print_size`, '');
          setValue(`materials.${index}.notes`, `Хэвлэлгүй форзац (${specs.endpaperDiv}ш гарна)`);
          const sheets = Math.ceil(a6 / specs.endpaperDiv);
          setValue(`materials.${index}.sheet_qty`, sheets);
          setValue(`materials.${index}.total_qty`, a6);
          setValue(`materials.${index}.extra_qty`, 0);
          return;
        }
        if (aux.type === 'endpaper_printed') {
          const epTotal = (a6 * specs.endpaperPrinted.baseMultiplier) + specs.endpaperPrinted.extra;
          const epSheets = Math.ceil(epTotal / specs.endpaperPrinted.divBy);
          setValue(`materials.${index}.print_size`, specs.endpaperPrinted.printSize);
          setValue(`materials.${index}.press_sheet`, specs.endpaperPrinted.pressSheet);
          setValue(`materials.${index}.extra_qty`, specs.endpaperPrinted.extra);
          setValue(`materials.${index}.total_qty`, epTotal);
          setValue(`materials.${index}.divide_by`, specs.endpaperPrinted.divBy);
        ...
        if (aux.type === 'capital') {
          ...
          setValue(`materials.${index}.sheet_qty`, capMeters);
          setValue(`materials.${index}.notes`, `Номын капитал (${a6}ш / ${specs.headbandDiv})`);
          setValue(`materials.${index}.extra_qty`, 0);
          return;
        }
        if (aux.type === 'ribbon') {
          const ribMeters = Math.ceil(a6 * specs.ribbonLength);
          setValue(`materials.${index}.sheet_qty`, ribMeters);
          setValue(`materials.${index}.notes`, `Хавчуурга тууз (${Math.round(specs.ribbonLength * 100)}см)`);
          setValue(`materials.${index}.extra_qty`, 0);
          return;
        }
        if (aux.type === 'strap') {
          setValue(`materials.${index}.sheet_qty`, a6 * 2);
          setValue(`materials.${index}.total_qty`, a6 * 2);
          setValue(`materials.${index}.extra_qty`, 0);
          return;
        }
  ...
  const handleAddHardcoverAuxiliary = (overrideOptions?: { isPrintedEndpaper?: boolean; hasRibbon?: boolean }) => {
    ...
    const endpaperDiv = specs.endpaperDiv;
    ...
    <div className="erp-main">
      ...
      <form onSubmit={handleSubmit(onSubmit)} onKeyDown={(e) => {
                  ...
                  materials.forEach((m, index) => {
                    ...
                    if (aux.isAux) {
                      ...
                      if (aux.type === 'endpaper_plain') {
                        setValue(`materials.${index}.extra_qty`, 0);
                        setValue(`materials.${index}.total_qty`, newBase);
                        setValue(`materials.${index}.sheet_qty`, Math.ceil(newBase / specs.endpaperDiv));
                        return;
                      }
                      if (aux.type === 'capital') {
                        setValue(`materials.${index}.extra_qty`, 0);
                        setValue(`materials.${index}.total_qty`, newBase);
                        setValue(`materials.${index}.sheet_qty`, Math.ceil(newBase / specs.headbandDiv));
                        return;
                      }
                      if (aux.type === 'ribbon') {
                        setValue(`materials.${index}.extra_qty`, 0);
                        setValue(`materials.${index}.total_qty`, newBase);
                        setValue(`materials.${index}.sheet_qty`, Math.ceil(newBase * specs.ribbonLength));
                        return;
                      }
                      if (aux.type === 'strap') {
                        setValue(`materials.${index}.extra_qty`, 0);
                        setValue(`materials.${index}.total_qty`, newBase * 2);
                        setValue(`materials.${index}.sheet_qty`, newBase * 2);
                        return;
                      }
                      if (aux.type === 'endpaper_printed') {
                        const epTotal = (newBase * specs.endpaperPrinted.baseMultiplier) + specs.endpaperPrinted.extra;
                        setValue(`materials.${index}.extra_qty`, specs.endpaperPrinted.extra);
                        setValue(`materials.${index}.total_qty`, epTotal);
                              ...
                              render={({ field }) => (
                                <Select
                                  ...
                                  onChange={(selectedOption: any) => {
                                    ...
                                    field.onChange(val);
                                    if (val) {
                                      const selectedAux = getMaterialType(val);
                                      const a7 = getA7Size();
                                      const totalQty = Number(getValues('total_qty')) || 1000;
                                      const isCover = Boolean(formValues.materials?.[index]?.is_cover);
                                      ...
                                      if (firstMatched) {
                                        ...
                                        setValue(`materials.${index}.unit_cost`, firstMatched.unit_cost);
                                      } else {
                                      ...
                                      if (selectedAux.isAux) {
                                        ...
                                        if (selectedAux.type === 'endpaper_plain') {
                                          ...
                                          setValue(`materials.${index}.extra_qty`, 0);
                                          setValue(`materials.${index}.total_qty`, totalQty);
                                          setValue(`materials.${index}.sheet_qty`, Math.ceil(totalQty / specs.endpaperDiv));
                                          return;
                                        }
                                        if (selectedAux.type === 'capital') {
                                          setValue(`materials.${index}.divide_by`, specs.headbandDiv);
                                          setValue(`materials.${index}.press_sheet`, '');
                                          setValue(`materials.${index}.print_size`, '');
                                          setValue(`materials.${index}.notes`, `Номын капитал (${totalQty}ш / ${specs.headbandDiv})`);
                                          setValue(`materials.${index}.base_qty`, totalQty);
                                          setValue(`materials.${index}.extra_qty`, 0);
                                          setValue(`materials.${index}.total_qty`, totalQty);
                                          setValue(`materials.${index}.sheet_qty`, Math.ceil(totalQty / specs.headbandDiv));
                                          return;
                                        }
                                        if (selectedAux.type === 'ribbon') {
                                          setValue(`materials.${index}.divide_by`, 1);
                                          setValue(`materials.${index}.press_sheet`, '');
                                          setValue(`materials.${index}.print_size`, '');
                                          setValue(`materials.${index}.notes`, `Хавчуурга тууз (${Math.round(specs.ribbonLength * 100)}см)`);
                                          ...
                                          setValue(`materials.${index}.extra_qty`, 0);
                                          setValue(`materials.${index}.total_qty`, totalQty);
                                          setValue(`materials.${index}.sheet_qty`, Math.ceil(totalQty * specs.ribbonLength));
                                          return;
                                        }
                                        if (selectedAux.type === 'strap') {
                                          setValue(`materials.${index}.divide_by`, 1);
                                          setValue(`materials.${index}.press_sheet`, '');
                                          setValue(`materials.${index}.print_size`, '');
                                          setValue(`materials.${index}.base_qty`, totalQty);
                                          setValue(`materials.${index}.extra_qty`, 0);
                                          setValue(`materials.${index}.total_qty`, totalQty * 2);
                                      ...
                                      if (!printSize) {
                                        printSize = getDefaultPrintSize(formValues.category, a7, isCover, bt, coverRules);
                                      ...
                                      setValue(`materials.${index}.sheet_qty`, Math.ceil(currentTotal / divBy));
                                    } else {
                                      setValue(`materials.${index}.size`, '');
                                      setValue(`materials.${index}.unit_cost`, 0);
                                    }
                      ...
                      <td style={{ padding: '0.25rem 0.3rem', borderRight: '1px solid #e2e8f0', verticalAlign: 'top' }}>
                        {aux.type === 'ctp' ? (
                          ...
                          <div 
                            ...
                            title="Хэвлэх машинд орохгүй, шууд зүсэгдэх туслах материал"
                          >
                            — (Хэвлэлгүй)
                          </div>
                        ) : (
                      ...
                      <td style={{ padding: '0.25rem 0.3rem', borderRight: '1px solid #e2e8f0', verticalAlign: 'top' }}>
                        {aux.type === 'ctp' ? (
                          ...
                          <div 
                            ...
                            title="Хэвлэлийн хуудас тооцохгүй"
                          >
        ...
        <SectionCard 
          ...
          sub="(Анхдагчаар 0₮ өртөгтэй цехийн технологийн зааварчилгаа болж явна. Өртөг тооцоонд оруулах бол Нэгж өртгийг бичнэ)"
          theme="amber"
          icon="🏭"
        >
          ...
          {opFields.length === 0 ? (
            <div style={{ padding: '1.75rem', textAlign: 'center', color: '#94a3b8', fontSize: '0.9rem', background: '#fff', border: '1px solid #e2e8f0', borderRadius: '8px' }}>
              Одоогоор ажиллагаа сонгогдоогүй байна. Дээрх "Нэмэлт ажиллагаа / Заавар сонгох" товчоор нэмнэ үү.
            </div>
          ) : (
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.85rem' }}>
                  <thead>
                    <tr style={{ background: '#f8fafc', borderBottom: '1px solid #cbd5e1' }}>
                      <th style={{ padding: '0.55rem 0.6rem', textAlign: 'left', color: '#475569', width: '135px' }}>Дамжлага</th>
                  ...
                  <tbody>
                    {opFields.map((field, index) => {
                      ...
                      const rawUnitCost = Number(o?.unit_cost) || 0;
                      const isPricing = rawUnitCost > 0;
                      const tCost = (Number(o?.qty) || 0) * rawUnitCost;
                      const mp = masterPrices.find(p => p.item_name === o?.operation_name);
                      const mpFormula = mp?.formula?.expression;
                      const suggestedPrice = mp?.unit_cost && mp.unit_cost > 0 ? mp.unit_cost : 0;
                      const inputStyle = tableInputStyle;

                      return (
                        <tr key={field.id} style={{ borderBottom: '1px solid #e2e8f0', background: isPricing ? '#fcfdfc' : '#fff' }}>
                          {/* 1. Production Stage */}
                          <td style={{ padding: '0.4rem 0.6rem', verticalAlign: 'middle' }}>
                            <select
                              ...
                              style={{ height: '28px', padding: '0 4px', fontSize: '0.78rem', borderRadius: '4px', border: '1px solid #cbd5e1', background: '#fff', width: '100%' }}
                            >
                          ...
                          <td style={{ padding: '0.4rem 0.6rem', verticalAlign: 'middle', fontWeight: 600, color: '#1e293b' }}>
                            ...
                            <input type="hidden" {...register(`operations.${index}.is_pricing`)} value={isPricing ? "true" : "false"} />
                          </td>
                            ...
                            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-end', gap: '4px' }}>
                              <input 
                                ...
                                style={{ 
                                  ...
                                  width: '85px', 
                                  textAlign: 'right',
                                  borderColor: isPricing ? '#86efac' : '#cbd5e1',
                                  background: isPricing ? '#f0fdf4' : '#fff',
                                  fontWeight: isPricing ? 700 : 400,
                                  color: isPricing ? '#166534' : '#475569'
                                }} 
                                {...register(`operations.${index}.unit_cost`, {
                                  onChange: (e) => {
                                    const val = Number(e.target.value) || 0;
                                    setValue(`operations.${index}.is_pricing`, val > 0);
                                  }
                                })} 
                              />
                              {suggestedPrice > 0 && (
                                <button
                                  type="button"
                                  onClick={() => {
                                    setValue(`operations.${index}.unit_cost`, suggestedPrice);
                                    setValue(`operations.${index}.is_pricing`, true);
                                  }}
                                  title={`Мастер сангийн жишиг үнэ (${suggestedPrice.toLocaleString()}₮) оруулах`}
                                  style={{
                                    background: '#f8fafc',
                                    border: '1px solid #cbd5e1',
                                    borderRadius: '4px',
                                ...
                                >
                                  💡 {suggestedPrice >= 1000 ? `${suggestedPrice / 1000}к` : suggestedPrice}₮
                                </button>
                              )}
                            </div>
                          </td>
                          ...
                          <td style={{ padding: '0.4rem 0.6rem', verticalAlign: 'middle', textAlign: 'right' }}>
                            {isPricing ? (
                              ...
                              <span style={{ color: '#64748b', fontSize: '0.78rem', background: '#f1f5f9', padding: '2px 6px', borderRadius: '4px', border: '1px solid #e2e8f0' }}>
                                0 ₮ (Заавар)
                              </span>
                            )}
                          </td>

                          {/* 6. Notes / Technical instruction */}
                          <td style={{ padding: '0.4rem 0.6rem', verticalAlign: 'middle' }}>
                            <input 
                              style={{ ...inputStyle, width: '100%' }} 
                              {...register(`operations.${index}.notes`)} 
                              placeholder="Цехийн ажилтанд зориулсан заавар / тэмдэглэл..." 
                            />
                          </td>
                          ...
                          <td style={{ padding: '0.4rem 0.6rem', verticalAlign: 'middle', textAlign: 'center' }}>
                            <button
                              ...
                              title="Устгах"
                              style={{ background: '#fee2e2', border: 'none', borderRadius: '4px', color: '#b91c1c', padding: '4px 8px', cursor: 'pointer', fontSize: '0.8rem' }}
                            >
                              ✕
                            </button>
                          </td>
